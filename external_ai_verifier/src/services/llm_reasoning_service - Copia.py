...
class ExternalLLMReasoningService:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.base_retry_delay = self.config.get('base_retry_delay', 1.0)  # segundos
        self.max_retry_delay = self.config.get('max_retry_delay', 60.0)   # segundos
    ...
    def analyze_with_llm(self, text: str, context: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            if self.provider == 'gemini':
                ...
                response = self._analyze_with_openai(prompt) # OpenAI não implementado com retry aqui, mas poderia
            else:
                return self._get_default_result()
    ...
    def _analyze_with_gemini_with_retry(self, prompt: str) -> str:
        """Análise com Gemini com lógica de retry para erros de cota."""
        last_exception = None
        delay = self.base_retry_delay

        for attempt in range(self.max_retries_on_quota_error + 1): # +1 para incluir a tentativa inicial
            try:
                ...
                # Verifica se é um erro de cota (429 ou mensagem contendo "quota", "rate limit")
                if "429" in error_message or "quota" in error_message or "rate limit" in error_message:
                    if attempt < self.max_retries_on_quota_error:
                        logger.warning(f"Erro de cota no Gemini (tentativa {attempt + 1}), aguardando {delay}s antes de tentar novamente: {e}")
                        time.sleep(delay)
                        ...
                        logger.error(f"Limite de tentativas excedido para erro de cota no Gemini: {e}")
                        break # Sai do loop após tentativas esgotadas
                else:
                    # Se não for erro de cota, não faz retry
                    logger.error(f"Erro não relacionado a cota no Gemini: {e}")
                    raise # Re-lança o erro imediatamente

        # Se o loop terminar e ainda houver uma exceção (de cota), relança-a
        if last_exception:
            raise last_exception

        # Este raise não deve ser alcançado se last_exception foi definido, mas é um fallback
